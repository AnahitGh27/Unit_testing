...
function unitTestingTask (format, date) {
  ...
  const dt = (arguments.length === 2 && date) ? date instanceof Date ? date : new Date(date) : new Date();
 ...
 * @type {Object}
 */
const languages = (unitTestingTask._languages = {});
 ...
 */
const lang = (unitTestingTask.lang = function (lang, options) {
  if (!lang) {
 ...
 */
const createFormatter = function (format) {
 ...
 * @return {Function} Readied formatting function with one argument — date.
 */
const register = (unitTestingTask.register = function (name, format) {
